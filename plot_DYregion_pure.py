...
def draw_plots(hist_array =[], draw_data=0, x_name='', isem=0, era = '2017'):
        ...
        plotdir = '/eos/user/t/tihsu/plot/Ctag_Shape/' + era + '_TTregion/'
...
#	QCD_yield =round(QCD.Integral(),1)
	...
	gr.Draw("SAME 2")
        if 'j1_CvB' in x_name:set_axis(h_stack,'x', 'CvB of leading jet', True)
        if 'j1_CvL' in x_name:set_axis(h_stack,'x', 'CvL of leading jet', True)
...
#        leg1.AddEntry(QCD,'QCD ['+str(QCD_yield)+']','f')
	...
	c.SaveAs(plotdir + x_name+'.png')
        ...
        pad1.SetLogy()
        c.Update()
        c.SaveAs(plotdir + x_name+'_log.pdf')
