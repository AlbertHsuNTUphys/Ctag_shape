...
def shape_cal(era):
  ...
  for process,desc in samples:
    ...
    for f in dirs:
      ...
      if(desc[1] and process == fname):
        f_SF = ROOT.TFile.Open(path + f)
        n_pre += f_SF.Get("OS_j1_CvsB_pre").Integral()
        n_reshape += f_SF.Get("OS_j1_CvB").Integral()
  return n_pre/n_reshape
...
def TTC_Analysis(era):
  ...
  for hist in hists_name:
    ...
    for process,desc in samples:
      ...
      if('pre' not in hist and h_SF is not None and desc[1]):
        h_SF.Scale(rescale)    
      histos.append(h_SF)      
